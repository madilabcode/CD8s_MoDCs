createCircosPlot <- function(LR = LR,
  ...
  #FC Ligands:
  FC_L <- FClegenedNcolor_Ligand(LR, fromCol)
  FC_L_fun <- FC_L[[1]]
  lgd_FC_L <- FC_L[[2]]
  FC_L_Data_Sub <- FC_L[[3]]
  ...
  # combine legends ligands
  #if(!is.null(Activation_DSA)){
  #  lgd_list_vertical_ligand = packLegend(lgd_LR_EXP.from, lgd_LR_EXP.to, lgd_FC_L, lgd_Activation_DSA, gap = unit(0.7, "cm")) 
  #}else{
  #  lgd_list_vertical_ligand = packLegend(lgd_LR_EXP.from, lgd_LR_EXP.to, lgd_FC_L, gap = unit(0.7, "cm")) 
  #}
  
  
  # combine legends receptors
  if(!is.null(Activation_DSA)){
    lgd_list_vertical = packLegend(lgd_LR_EXP.from, lgd_LR_EXP.to,lgd_FC_L, lgd_FC, lgd_Activation_DSA, gap = unit(0.7, "cm")) 
  }else{
    lgd_list_vertical = packLegend(lgd_LR_EXP.from, lgd_LR_EXP.to,lgd_FC_L ,lgd_FC, gap = unit(0.7, "cm")) 
  }
  ...
  #the plot itself
  pdf(Figure_name, width=14, height=11)
  circos.clear()
  ...
  chordDiagram(LR,
               ...
               preAllocateTracks = list(track.height = LR %>% dimnames() %>% unlist() %>% strwidth() %>% max())) 
                ...
                # FC Ligand loop  
                for(ligand in base::intersect(FC_L_Data_Sub %>% rownames(), LR$Ligand)){circos.rect(
                  xleft = 0,
                  xright = c(grep(paste0("^",ligand,"$"),LR$Ligand)) %>% length()-0.1,
                  ybottom = 0.95, ytop = 1.15, 
                  col    = FC_L_fun(FC_L_Data_Sub[ligand,"log2FoldChange"]), 
                  border = FC_L_fun(FC_L_Data_Sub[ligand,"log2FoldChange"]),
                  sector.index = ligand, track.index = 1)}
  
  
                # FC Receptor loop  
                for(receptor in base::intersect(FC_Data_Sub %>% rownames(), LR$Receptor)){circos.rect(
                  xleft = 0,
                ...
                if(!is.null(Activation_DSA)){
                #Activation DSA loop
                  for(receptor in base::intersect(Activation_DSA_Data_Sub$Receptor, LR$Receptor)){
                    if (Activation_DSA_Data_Sub[receptor, "fdr_value"] < 0.05) {
                      circos.rect(
                      xleft = 0,
  ...
  #draw(lgd_list_vertical_ligand, just = c("left", "up"), x = unit(1, "cm"), y = unit(1, "cm"))
  title(main = title, line = 0, adj = 0.5)
  title(main = subtitle, line = -1, adj = 1, cex=0.7)
