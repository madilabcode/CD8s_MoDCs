FClegenedNcolor <- function(LR, toCol){
  ...
  DE_Data_R <- toCol %>% filter(toCol %>% row.names() %in% LR$Receptor)
  Max <- DE_Data_R$log2FoldChange %>% max(na.rm = T) %>% round(2)
  DEcol_fun = colorRamp2(c(0, Max), c("white", "#28a10a"))
...
DSAcalculator <- function(Gene ,RNAseqEXPCol, threshold = 100, directed_mouse_ppi){
  ...
  if(ReceptorTBActive$mode %>% length() > 0){
    ...
    inter <- intersect(ReceptorTB$`Output-node Gene Symbol`, ReceptorTBActive$item_id_a) %>% as.vector()
    if(inter %>% length() > 0){
      DSA <- RNAseqEXPCol[intersect(RNAseqEXPCol %>% rownames, inter),] %>% mean %>% round(1)
      n <- length(inter)
...
EXPlegenedNcolorFrom <- function(LR, z_scores = F){
  GeneExpressionFrom <- LR %>% select(Ligand, LigandAbundance) %>% unique()
  ...
  if(z_scores == T){
      ...
      GeneExpressionFrom <- GeneExpressionFrom %>% select(-c(LigandAbundance))
  }else{
...
EXPlegenedNcolorTo <- function(LR, z_scores = F){
  GeneExpressionTo <- LR %>% select(Receptor, ReceptorExp) %>% unique()
  ...
  if(z_scores == T){
    ...
    GeneExpressionTo <- GeneExpressionTo %>% select(-c(ReceptorExp))
  }else{
