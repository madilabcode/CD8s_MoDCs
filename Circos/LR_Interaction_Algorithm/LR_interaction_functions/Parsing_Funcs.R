...
ReceptorParser <- function(ReceptorDE_Res, cell_line, FC_thresh){
  ...
  ReceptorDE_ResSubOrd <- ReceptorDE_ResSub %>% dplyr::arrange(desc(log2FoldChange))
  ReceptorDE_ResSubOrdUni <- ReceptorDE_ResSubOrd[!(ReceptorDE_ResSubOrd$gene_name %>% duplicated()),] %>% na.omit()
  DE_Receptors <- ReceptorDE_ResSubOrdUni %>% dplyr::filter(log2FoldChange > FC_thresh & padj < 0.05) %>%
                        remove_rownames() %>% column_to_rownames('gene_name') 
  DE_Receptors <- DE_Receptors %>% dplyr::select(log2FoldChange) 
  return(DE_Receptors)
}

RNAseqEXPParser <- function(RNAseqEXPMean, cell_line){
  RNAseqEXPMeanSub <- RNAseqEXPMean %>% dplyr::select(c('gene_name',cell_line)) 
  RNAseqEXPMeanSubOrd <- RNAseqEXPMeanSub %>% dplyr::arrange(desc(get(cell_line)))
