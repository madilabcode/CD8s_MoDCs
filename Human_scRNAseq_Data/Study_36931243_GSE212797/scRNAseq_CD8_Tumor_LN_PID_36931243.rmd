...
ggsave(plot = Feature_plot,file = ".\\Filtered\\Features_SCRNA_Func.pdf", dpi=300, width=7, height=4) #8 8
...
cytotoxic_boxplot <- ggplot(Cytotoxic_score_data, aes(x = tissue, y = cytotoxic_sig1)) +  
  geom_boxplot(fill = color_pal, color = "black", width = 0.25) +
...
#Violin plot
cytotoxic_enriched_vln <- VlnPlot(cluster_2_subset_sig_scores, features = "cytotoxic_sig1", group.by = "tissue") + labs(x = "", y = "Cytotoxic Signature Enrichment Score") +  theme_minimal()+ ggtitle("Violin plot of Cytotoxic Signature Enrichment Score In The Tumor And LN") + theme(plot.title = element_text(hjust = 0.5)) + ylim(min(cluster_2_subset_sig_scores$cytotoxic_sig1),max(cluster_2_subset_sig_scores$cytotoxic_sig1) + 0.5) + geom_signif(comparisons = list(c("tumor", "lymph node")), 
              map_signif_level = T)
ggsave(plot = cytotoxic_enriched_vln, file = ".\\Filtered\\Cytotoxicity_Enriched_Violin_Plot.pdf", dpi=300, width=8, height=8)
